#!/bin/bash
# Automated script to fix server - will prompt for passphrase
...
echo "You'll be prompted for your SSH key passphrase once."
echo ""

# Use ssh with ControlMaster for connection reuse
ssh -o ControlMaster=auto -o ControlPath=/tmp/ssh-%r@%h:%p -o ControlPersist=60s \
    -i "$SSH_KEY" "$SERVER" << 'ENDSSH'
...
echo ""
echo "✅ Done! Check PM2 logs with: pm2 logs soundope-api"
ENDSSH
